...
for (colname in names(indoordata)) {
    if (grepl("indoor_", colname, fixed = TRUE)) {
        ...
        outdoor <- joineddata[paste0("outdoor_", datatype)]
...
# NOTE: NONE OF THIS WORKS. NEED SOME NEEL MAGIC

#hepamean <- mean(hepa_installed, na.rm = TRUE)


 fifth_hepa <- apply(hepa_installed, 2, median, na.rm = TRUE)
 fifth_nohepa <- apply(no_hepa_installed, 2, median, na.rm = TRUE)




# joineddata %>%
#   group_by(joineddata$hepa_installed) %>%
#   summarise(fifth = quantile(indoor_bin0, probs = 0.05))



# fifth <-  quantile(hepa_installed, .05, na.rm = TRUE)
# twentyfifth <- quantile(hepa_installed, .25, na.rm = TRUE)
