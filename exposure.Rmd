...
df_sensor_sn %>%
  ...
  geom_hline(yintercept = df_hepa_stats_bin0$twentyfifth[1], linetype = "dashed", color = "green") +
...
df_sensor_sn %>%
  ...
  geom_hline(yintercept = df_hepa_stats_bin0$twentyfifth[2], linetype = "dashed", color = "green") +
...
pm_baseline_reduction["binint"] <- as.double(sub(".*?(\\d+)", "\\1", pm_baseline_reduction$bin))

pm_baseline_reduction[, "binint"][pm_baseline_reduction[, "binint"] == 25.0] <- 2.5
